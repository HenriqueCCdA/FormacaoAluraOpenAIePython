...
from helper_models import MODELE_GPT_SCHEMA
...
def abrir_dados_csv(cliente, nome_arquivo):
    if not os.path.exists(nome_arquivo):
        nome_arquivo = cliente.files.retrieve(extrair_id_arquivo(nome_arquivo))
...
def relatorio_complexidade_algoritmos(argumentos):
    ...
    from ia_chat import criar_assistente
...
def calcular_complexidade_tempo(argumentos):
    ...
    nome = "Assistente de complexidade de Tempo"
    descricao_agente = """
    Este assistente é projetado para analisar e retornar a complexidade de
    um algoritmo utilizando a notação Big O. Ele fornece uma análise
    detalhada, incluindo a complexidade do aalgoritmo, o processo utilizado
    para verificar essa complexidade, quando a solução pode se tornar um problema,
    e possíveis soluções computacionais alternativas

    **Instruções de Uso:**

    Baseado na obra "Introduction to Algorithms" de Thomas H. Cormen et al., este assistente aplicará
    os conceitos de análise de algoritmos para determinar a complexidade de tempo de uma função ou método específico.
    Ele utiliza técnicas como a análise de loops aninhados, recorrência de Mestre, e outras abordagens
    consagradas para garantir uma avaliação precisa.

    1. **Complexidade do Algoritmo**: O assistente determinará a complexidade do algoritmo analisado
    e retornará no formato Big O. Exemplos incluem O(n), O(log n), O(n^2), entre outros.


    2. **Processo de Verificação**: O assistente explicará como a complexidade foi determinada,
    utilizando técnicas como análise de loops, recursões, e chamadas recursivas.
    Referências teóricas incluem a análise de loops aninhados para complexidade O(n^2) e a
    aplicação da recorrência de Mestre para algoritmos recursivos.

    3. **Identificação de Problemas**: O assistente indicará quando a solução pode se tornar ineficiente,
    especialmente em cenários com grandes volumes de dados ou limitações de tempo/espacial.
    Ele avaliará a escalabilidade com base em casos de teste padronizados,
    como a classificação de 10^6 elementos ou a busca em grafos densos.

    4. **Soluções Computacionais Conhecidas**: O assistente sugerirá possíveis otimizações,
    como a aplicação de técnicas de memoização, uso de algoritmos alternativos
    (como substituição de uma pesquisa linear por uma pesquisa binária),
    ou melhorias na estrutura de dados (como substituição de listas por heaps).
    Exemplos comuns incluem a substituição de um algoritmo de ordenação O(n^2)
    por QuickSort ou MergeSort com complexidade O(n log n).
    """

    ferramenta = Assistente(nome=nome, instrucoes=descricao_agente, eh_ferramenta=True)

    resposta = ferramenta.perguntar(
        pergunta=f"Faça uma análise completa do método {metodo_avaliado} do script com nome: {caminho_script}",
        caminho_arquivo=caminho_script,
    )

    ferramenta.apagar_agente()

    print(f"Respota da ferramenta", resposta)
...
def avaliar_adequacao_pep8(argumentos):
    ...
    resposta = ferramenta.perguntar(
        pergunta=f"Faça uma análise de conformidade ao PEP8 para o script com nome: {caminho_script}. Liste todos os casos possiveis",
        caminho_arquivo=caminho_script,
    )
