...
def salvar_resposta_ia_binario(caminho_arquivo, dados, tipo_resposta=""):
    ...
    nome_aquivo_sem_extensao = os.path.splitext(nome_com_extensao)[0]

    caminho_novo = f"respostas/{tipo_resposta}-{nome_aquivo_sem_extensao}.py"
...
def salvar_resposta_ia(caminho_arquivo, dados, tipo_resposta=""):
    ...
    caminho_novo = f"respostas/{tipo_resposta}-{nome_aquivo_sem_extensao}.txt"
...
def calcular_diferencas(conteudo_anterior, conteudo_atual):
    diff = difflib.unified_diff(
        conteudo_anterior.splitlines(keepends=True),
